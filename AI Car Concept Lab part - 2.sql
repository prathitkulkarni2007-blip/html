...
|―— static/
 ...
 groq==1.0.0
 python-dotenv==1.2.1
 FLASK_SECERT_KEY=replace_with_any_long_random_string
 GROQ_API_KEY=your_grog_key_here
 GROQ_MODEL+llama-3.3-70b-versatile
 Important
 Create a second file named exactly.env in the same folder.Copy the keys into it and replace GROQ API KEY with your key only if you want live GROQ output.
...
def get_prefs(data: Any) -> dict[str, str]:
    ...
    return {k: str(getattr(data, 'get', lambda *_: d)(k, d)).strip() for k, d in fields}
...
def normalize_colorways(colorways: list[dict[str, Any]], brief: dict[str, str]) -> list[dict[str, str]]:
    fallback = [
        ...
        {"name": "Solar Echo", "body": shift_hex(brief["primary_color"], 18), "accent": shift_hex(brief["accent_color"], 20), "wheels": "#2b2f3e", "ca
 bin": "#f4f1eb"},
    ]
...
<section class="shell studio-shell" id="studioApp">
  <div class="panel brief-panel">
    ...
    <form id="conceptForm" class="brief-form">
      ...
      <p class="form-note">Same brief, different concept is expected here. The AI is tuned for creative variety, not repeated clones.</p>
    </form>
  </div>
...
python app.py
Home Page:
https://locolhost:5000/


Studio Page:
https://locolhost:5000/studio


History Page:
https://locolhost:5000/history