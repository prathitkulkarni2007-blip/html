...
CAPTCHA_ENABLED = bool(HCAPTCHA_SITE_KEY and HCAPTCHA_SECRET)
...
FIELDS = {
    "style": "Hypercar", "material": "Carbon Fiber", "occasion": "City Tech",
    "primary_color": "#1d4ed8", "accent_color": "#f97316",
...
MATERIALS = {
    ...
    "Titanium": ["titanium highlights", "heat-blued metal details", "ultra-premium spine"],
...
PRICES = {
    "Daily Drive": "$58,000", "Track Day": "$210,000", "Adventure": "$96,000",
    "Collector Reveal": "$320,000", "City Tech": "$74,000", "Luxury Tourer": "$168,000"
}
...
def generate():
    ...
    if config.CAPTCHA_ENABLED and not verify_hcaptcha(token):
        return fail("CAPTCHA verification failed. Refresh and try again.", 400)
    prefs = {k: str(data.get(k, v)).strip() or v for k, v in FIELDS.items()}
    try:
        concept = make_concept(prefs)
    except json.JSONDecodeError as e:
        return fail(f"Malformed AI response: {e}", 500)
    except Exception as e:
        return fail(f"Concept generation failed: {e}", 500)
    concept.update(
...
def generate_image():
    prompt = str((request.get_json(silent=True) or {}).get("image_prompt", "")).strip()
    if not prompt:
        return fail("No image prompt.", 400)
    if not config.HF_API_KEY:
        return fail("HF_API_KEY not configured.", 503)
    image_url = hf_image(prompt)
    return jsonify(success=True, image_url=image_url) if image_url else fail("Image generation failed. Try again.", 500)
...
def verify_hcaptcha(token: str) -> bool:
    ...
    if not token:
        return False
    try:
        res = requests.post(config.HCAPTCHA_VERIFY_URL, data={"secret": config.HCAPTCHA_SECRET, "response": token}, timeout=5)
        return res.json().get("success", False)
    except Exception:
        return False

def make_concept(prefs: dict[str, str]) -> dict:
    if client:
        ...
        parsed = normalize(json.loads(extract_json(raw)), prefs)
        if parsed:
            return parsed
...
def normalize(data: dict, brief: dict[str, str]) -> dict:
    ...
    return {
        ...
        "colorways": clean_colorways(data.get("colorways", []), brief),
    }

def clean_colorways(items: list[dict], brief: dict[str, str]) -> list[dict[str, str]]:
    fallback = [
        {"name": "Launch Spec", "body": brief["primary_color"], "accent": brief["accent_color"], "wheels": "#1b1e28", "cabin": "#d9dce5"},
        {"name": "Midnight Pulse", "body": tint(brief["primary_color"], -18), "accent": brief["accent_color"], "wheels": "#0f1218", "cabin": "#c7d2e2"},
        {"name": "Solar Echo", "body": tint(brief["primary_color"], 18), "accent": tint(brief["accent_color"], 20), "wheels": "#2b2f3e", "cabin": "#f4f1eb"},
    ]
    clean = [{"name": str(item.get("name", fallback[i]["name"])).strip(),"body": as_hex(item.get("body"), fallback[i]["body"]),"accent": as_hex(item.get("accent"), fallback[i]["accent"]),"wheels": as_hex(item.get("wheels"), fallback[i]["wheels"]),"cabin": as_hex(item.get("cabin"), fallback[i]["cabin"])} for i, item in enumerate(items[:3])]
    return clean + fallback[len(clean):]

def as_hex(value, fallback):
    value = f"#{str(value).strip().lstrip('#')}"
    return value if re.fullmatch(r"#[0-9a-fA-F]{6}", value) else fallback

def tint(hex_color: str, amount: int) -> str:
    parts = [int(as_hex(hex_color, "#667eea")[i:i + 2], 16) for i in (1, 3, 5)]
...
def local_concept(brief: dict[str, str]) -> dict:
    ...
    return normalize({"name": name,"tagline": rng.choice(["Built for tomorrow's roads.", "Where concept meets velocity.", "Electric drama. Precision control.", "Designed to steal the skyline."]),"description": f"{name} is a {brief['style'].lower()} concept shaped for {brief['occasion'].lower()} moments. It pairs {brief['material'].lower()} surfaces with {brief['primary_color']} as the hero tone and {brief['accent_color']} as the visual pulse. The whole build channels {brief['inspiration']} into a cinematic road presence.","materials": MATERIALS.get(brief["material"], MATERIALS["Mixed Performance"])[:3],"features": [f"{brief['style']} silhouette with active aero channels", f"{brief['occasion']} tuned suspension package", f"Ambient interface inspired by {brief['inspiration']}", "Adaptive torque vectoring control"],"powertrain": rng.choice(["Tri-motor electric AWD", "Hydrogen-electric hybrid drive", "Dual-motor instant torque system", "Solid-state battery performance core"]),"cockpit_theme": rng.choice(["Wraparound holographic cockpit", "Lounge-tech cabin with floating controls", "Driver-centric digital command pod", "Minimal tactile cockpit with adaptive ambient light"]),"target_driver": rng.choice(["Collectors chasing concept drama", "Urban futurists who love design", "Performance-first EV adopters", "Luxury-tech explorers"]),"retail_price": PRICES.get(brief["occasion"], "$120,000"),"style_tags": STYLE_TAGS.get(brief["style"], STYLE_TAGS["EV"])[:3],"colorways": []}, brief)

def build_image_prompt(prefs):
    p = hex_to_color_name(prefs["primary_color"])
    a = hex_to_color_name(prefs["accent_color"])
    prompt = (f"Professional product photography of a futuristic {prefs['style']} concept car, {p} body finish, {a} accent highlights, {prefs['material']} surfaces, three-quarter front view, studio lighting, white background, sharp focus, 8k, car only")
...
def hf_image(prompt):
    try:
        res = requests.post(config.HF_IMAGE_URL, headers={"Authorization": f"Bearer {config.HF_API_KEY}", "Content-Type": "application/json"}, json={"inputs": prompt}, timeout=60)
        if res.status_code == 200 and res.headers.get("content-type", "").startswith("image"):
            mime = res.headers["content-type"].split(";")[0].strip()
...
def placeholder_image(name: str) -> str:
    label = requests.utils.quote(name[:26])
    return f"https://placehold.co/1200x720/0f172a/e2e8f0?text={label}"

def fingerprint(prefs: dict[str, str], name: str) -> str:
    raw = json.dumps({"prefs": prefs, "name": name}, sort_keys=True).encode()
...
<head>
  ...
  <script>window.HCAPTCHA_SITE_KEY = {{ hcaptcha_site_key|default('', true)|tojson }};</script>
  {% if hcaptcha_site_key %}<script src="https://js.hcaptcha.com/1/api.js?render=explicit&onload=hcaptchaReady" async defer></script>{% endif %}
</head>
<body>
  <div class="noise"></div>
  ...
  <footer class="site-footer"><p>Built with Flask, Groq concept generation, optional hCaptcha protection, Hugging Face image rendering, and instant SVG colourway switching.</p></footer>
...
{% block content %}
<section class="hero shell"> ... keep the landing page from the working project zip, but update the wording so the History button points to the next part rather than active history. ... </section>
{% endblock %}
      ...
      <div class="media-grid">
        ...
        <div class="svg-card">
          <div class="svg-toolbar">
            <strong>Instant Colorway Explorer</strong>
            <div id="swatchButtons" class="swatch-buttons"></div>
          </div>
          <div id="carSvgMount"></div>
          <div id="colorwayInfo" class="cw-info-grid"></div>
        </div>
      </div>
...
<section class="shell history-shell">
  <div class="history-head"><div><span class="eyebrow">Next Part</span><h1>History will be activated in the next lesson.</h1><p>This part focuses only on concept generation, optional CAPTCHA, image rendering, and the SVG colourway explorer.</p></div></div>
  <div class="empty-history glow-card"><div class="emoji">🚗</div><h2>History is not included in this part.</h2><p>Generate concepts in the Studio now. Session-based or browser-based history can be added cleanly in the next part.</p><a href="{{ url_for('studio') }}" class="btn btn-primary">Open Studio</a></div>
</section>
{% endblock %}
...
.cw-swatch{width:14px;height:14px;border-radius:50%;display:inline-block;border:1px solid rgba(255,255,255,.22)}
.swatch-btn{display:inline-flex;align-items:center;gap:8px}
.cw-info-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:14px}
.cw-color-item{display:flex;align-items:center;gap:10px;padding:12px;border:1px solid var(--line);border-radius:14px;background:rgba(255,255,255,.03);color:#dbeafe;font-size:13px}
.cw-dot{width:14px;height:14px;border-radius:50%;border:1px solid rgba(255,255,255,.18);flex:0 0 14px}
.car-svg{width:100%;height:auto;display:block}

(() => {
  const $ = id => document.getElementById(id)
  const form = $('conceptForm'), btn = $('generateBtn'), err = $('formError')
  const empty = $('emptyState'), load = $('loadingState'), result = $('resultState')
  const loader = $('loaderText'), modal = $('captchaModal'), cancel = $('cancelCaptcha')
  const imgStatus = $('imageStatus'), regen = $('regenerateImageBtn')
  const tabs = $('swatchButtons'), info = $('colorwayInfo'), mount = $('carSvgMount')
  const widgetHost = $('hcaptchaWidget')
  const MSGS = ['Querying AI…', 'Sketching body lines…', 'Balancing materials…', 'Dialing the cockpit mood…', 'Finalizing launch specs…']
  let ticker = null, widget = null, concept = null, colorways = []

  const esc = s => String(s ?? '').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))
  const set = (id, v) => ($(id).textContent = v || '')
  const list = (id, arr) => ($(id).innerHTML = (arr || []).map(v => `<li>${esc(v)}</li>`).join(''))
  const tags = (id, arr) => ($(id).innerHTML = (arr || []).map(v => `<span class="tag">${esc(v)}</span>`).join(''))
  const rgba = (hex, a) => { const n = parseInt((hex || '#000000').slice(1), 16); return `rgba(${n >> 16},${(n >> 8) & 255},${n & 255},${a})` }
  const shade = (hex, by) => '#' + [16, 8, 0].map(s => Math.max(0, Math.min(255, ((parseInt((hex || '#000000').slice(1), 16) >> s) & 255) + by)).toString(16).padStart(2, '0')).join('')
  const show = key => [['empty', empty], ['loading', load], ['result', result]].forEach(([name, el]) => el.classList.toggle('hidden', name !== key))
  const start = () => { let i = 0; loader.textContent = MSGS[0]; ticker = setInterval(() => loader.textContent = MSGS[i = (i + 1) % MSGS.length], 1400) }
  const stop = () => { clearInterval(ticker); ticker = null }
  const hideModal = () => modal?.classList.add('hidden')
  const failCaptcha = msg => { hideModal(); btn.disabled = false; err.textContent = msg }

  window.hcaptchaReady = () => {
    if (!window.hcaptcha || !widgetHost || widget !== null || !window.HCAPTCHA_SITE_KEY) return
    widget = window.hcaptcha.render('hcaptchaWidget', {
      sitekey: window.HCAPTCHA_SITE_KEY,
      theme: 'dark',
      size: 'compact',
      callback: token => run(token),
      'expired-callback': () => failCaptcha('CAPTCHA expired. Please try again.'),
      'error-callback': () => failCaptcha('CAPTCHA failed to load. Refresh and try again.')
    })
  }
  if (window.hcaptcha) window.hcaptchaReady()

  form?.addEventListener('submit', e => {
    e.preventDefault(); err.textContent = ''
    if (window.HCAPTCHA_SITE_KEY) {
      if (window.hcaptcha && widget === null) window.hcaptchaReady()
      if (window.hcaptcha && widget !== null) {
        btn.disabled = true
        window.hcaptcha.reset(widget)
        modal?.classList.remove('hidden')
        return
      }
      err.textContent = 'CAPTCHA is still loading. Please wait a moment and try again.'
      return
    }
    run('')
  })
  cancel?.addEventListener('click', () => (hideModal(), btn.disabled = false))
  regen?.addEventListener('click', () => concept?.image_prompt && fetchImage(concept.image_prompt))

  async function run(token) {
    err.textContent = ''; hideModal(); btn.disabled = true; show('loading'); imgStatus.textContent = ''; start()
    try {
      const prefs = Object.fromEntries(new FormData(form).entries())
      const res = await fetch('/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...prefs, 'h-captcha-response': token }) })
      const data = await res.json()
      if (!res.ok || !data.success) throw new Error(data.error || 'Something went wrong.')
      concept = data.concept
      render(concept)
      show('result')
      fetchImage(concept.image_prompt)
    } catch (e) {
      show('empty')
      err.textContent = e.message || 'Could not generate the concept.'
    } finally {
      stop(); btn.disabled = false
      if (window.hcaptcha && widget !== null) window.hcaptcha.reset(widget)
    }
  }

  async function fetchImage(prompt) {
    if (!prompt) return
    imgStatus.textContent = 'Rendering image… This can take a little longer.'
    if (regen) regen.disabled = true
    try {
      const res = await fetch('/generate-image', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image_prompt: prompt }) })
      const data = await res.json()
      if (!res.ok || !data.success) throw new Error(data.error || 'Image generation failed.')
      $('conceptImage').src = data.image_url
      imgStatus.textContent = 'AI render ready.'
    } catch (e) {
      imgStatus.textContent = e.message || 'Image generation failed.'
    } finally {
  ...
  function render(c) {
    ['carName:name','tagline:tagline','retailPrice:retail_price','description:description','powertrain:powertrain','cockpitTheme:cockpit_theme','targetDriver:target_driver'].forEach(x => { const [id, key] = x.split(':'); set(id, c[key]) })
    list('materials', c.materials); list('features', c.features); tags('styleTags', c.style_tags)
    $('conceptImage').src = c.image_url; $('conceptImage').alt = `${c.name} concept render placeholder`
    colorways = c.colorways || []; renderSvg(c.name); buildTabs(colorways); pick(0)
  }

  function renderSvg(name = '') {
    mount.innerHTML = `<svg class="car-svg" viewBox="0 0 780 420" role="img" aria-label="${esc(name || 'Car concept')} colourway preview"><defs><linearGradient id="bodyFill" x1="0" x2="1"><stop offset="0%" stop-color="var(--body-color,#1d4ed8)"/><stop offset="100%" stop-color="var(--body-glow,#4f7fff)"/></linearGradient><linearGradient id="accentFill" x1="0" x2="1"><stop offset="0%" stop-color="var(--accent-color,#f97316)"/><stop offset="100%" stop-color="var(--accent-glow,#ffb26b)"/></linearGradient></defs><rect width="780" height="420" rx="28" fill="#0d1220"/><ellipse cx="390" cy="336" rx="250" ry="24" fill="var(--shadow-color,rgba(249,115,22,.18))"/><path d="M128 264 Q176 176 314 160 L468 146 Q576 147 642 223 L684 263 L660 292 L598 292 Q582 239 530 236 Q480 238 458 292 L276 292 Q254 238 204 236 Q151 239 132 292 L98 292 Q75 292 83 270 Z" fill="url(#bodyFill)" stroke="var(--body-stroke,#7ca2ff)" stroke-width="4"/><path d="M285 168 L362 132 Q437 118 516 128 L582 214 L352 214 Z" fill="var(--cabin-color,rgba(217,220,229,.86))"/><path d="M170 248 L102 260" stroke="url(#accentFill)" stroke-width="8" stroke-linecap="round"/><path d="M614 247 L674 256" stroke="#fff2b0" stroke-width="8" stroke-linecap="round"/><path d="M323 158 Q416 135 526 146" stroke="var(--accent-line,rgba(249,115,22,.95))" stroke-width="6" stroke-linecap="round"/><circle cx="208" cy="290" r="43" fill="var(--wheel-color,#1b1e28)"/><circle cx="208" cy="290" r="22" fill="var(--wheel-inner,#5a6070)"/><circle cx="528" cy="290" r="43" fill="var(--wheel-color,#1b1e28)"/><circle cx="528" cy="290" r="22" fill="var(--wheel-inner,#5a6070)"/><text x="40" y="52" fill="#dbeafe" font-size="24" font-family="Inter, Arial" font-weight="700">${esc(name || 'Launch Spec')}</text><text id="svgSpecLine" x="40" y="82" fill="#95a4c6" font-size="14" font-family="Inter, Arial">Choose a colourway</text></svg>`
  }
  ...
  function buildTabs(arr) {
    ...
    arr.forEach((cw, i) => {
      ...
      b.type = 'button'; b.className = `swatch-btn${i ? '' : ' active'}`
      b.innerHTML = `<span class="cw-swatch" style="background:${cw.body}"></span><span class="cw-swatch" style="background:${cw.accent}"></span>${esc(cw.name)}`
      b.onclick = () => pick(i)
      tabs.appendChild(b)
    })
  }
  ...
  function paint(cw) {
    const svg = mount.querySelector('.car-svg'); if (!svg || !cw) return
    Object.entries({ '--body-color': cw.body, '--body-glow': shade(cw.body, 32), '--body-stroke': shade(cw.body, 52), '--accent-color': cw.accent, '--accent-glow': shade(cw.accent, 18), '--accent-line': rgba(cw.accent, .95), '--wheel-color': cw.wheels, '--wheel-inner': shade(cw.wheels, 70), '--cabin-color': rgba(cw.cabin, .86), '--shadow-color': rgba(cw.accent, .18) }).forEach(([k, v]) => svg.style.setProperty(k, v || ''))
    svg.querySelector('#svgSpecLine').textContent = `Body · ${cw.body || ''} · Accent · ${cw.accent || ''}`
  }

  function pick(idx) {
    tabs.querySelectorAll('.swatch-btn').forEach((el, i) => el.classList.toggle('active', i === idx))
    const cw = colorways[idx]; paint(cw)
    info.innerHTML = [['Body', cw?.body], ['Accent', cw?.accent], ['Wheels', cw?.wheels], ['Cabin', cw?.cabin]].map(([label, color]) => `<div class="cw-color-item"><span class="cw-dot" style="background:${color || '#000'}"></span>${label}: <strong>${esc(color || '')}</strong></div>`).join('')
  }
})()
